rm(list = ls())
setwd("E/:Pitts/ECON 2813 Global Econ and Fin")
...
head(Y)
tail(Y)
keep_data <- seq(from = as.Date("1947-04-01"), to = as.Date("2019-10-1"), by = "quarter")
...
ACF = acf(DY_new,lag=round(n_obs^(1/3))) # command to obtain sample ACF of the data
TACF <- ARMAacf(ar_coeff, ma_coeff, lag.max = round(n_obs^(1/3))) # command to obtain theorical ACF
plot(c(0:round(n_obs^(1/3))),ACF$acf,type='l',xlab='Lag',ylab='ACF',ylim=c(0,1))
lines(0:round(n_obs^(1/3)),TACF,lty=2)
grid(nx = 4, ny = 4)
